...
-- bge-m3 produces 1024-dimensional multilingual embeddings and is suitable
-- for Arabic/English local retrieval. Change this dimension only through a
-- migration if a different embedding model is selected later.
...
create index if not exists ix_documents_repository_entry
    on public.documents (repository_id, entry_id);
...
create index if not exists ix_documents_source_modified
    on public.documents (source_modified_at desc);
...
create index if not exists ix_document_metadata_field_value
    on public.document_metadata (field_name, value_text);
...
drop trigger if exists document_metadata_set_updated_at on public.document_metadata;
